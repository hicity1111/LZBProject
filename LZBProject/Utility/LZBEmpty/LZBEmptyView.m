...
- (void)setSubViewFrame{
    ...
    if (self.ignoreContentInset && [self.superview isKindOfClass:[UIScrollView class]]) {
        ...
        self.centerY += scrollView.contentInset.bottom;
        self.centerX += scrollView.contentInset.right;
    }
}
...
- (void)setupTitleLabel:(NSString *)titleStr{
    ...
    contentHeight = self.titleLabel.frame.origin.y + self.titleLabel.height;
}
...
- (void)setupDetailLabel:(NSString *)detailStr{
    ...
    contentWidth = width > contentWidth ? width : contentWidth;
    contentHeight = self.detailLabel.mj_y + self.detailLabel.height;
   
}

- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(LZBActionTapBlock)btnClickBlock{
    ...
    contentHeight = self.actionButton.mj_y + self.actionButton.height;
}
