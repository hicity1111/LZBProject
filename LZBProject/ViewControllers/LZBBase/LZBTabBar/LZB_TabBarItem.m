...
- (instancetype)initWithModel:(LZB_TabBarConfigModel *)itemModel{
    self = [super init];
    if (self) {
        [self configSelf];
...
- (void)itemDidLayoutBadgeLabel{
    switch (self.itemModel.itemBadgeStyle) {
        case LZB_TabBarItemBadgeStyleTopRight:{   // 右上方 默认
            
            self.badgeLabel.center = CGPointMake(self.frame.size.width/2 + self.frame.size.height/2 ,self.badgeLabel.frame.size.height/2 + 3.5);
        }
            break;
        case LZB_TabBarItemBadgeStyleTopCenter:{      // 上中间
            self.badgeLabel.center = CGPointMake(self.frame.size.width/2,self.badgeLabel.frame.size.height/2 );
        }
            break;
        case LZB_TabBarItemBadgeStyleTopLeft:{         // 左上方
...
- (void)itemDidLayoutControl{
    ...
    switch (self.itemModel.itemLayoutStyle) {
        case LZB_TabBarItemLayoutStyleTopPictureBottomTitle:{         // 上图片下文字 使用图3 文1比
        ...
        case LZB_TabBarItemLayoutStyleBottomPictureTopTitle:{         // 下图片上文字
        ...
        case LZB_TabBarItemLayoutStyleLeftPictureRightTitle:{         // 左图片右文字
        ...
        case LZB_TabBarItemLayoutStyleRightPictureLeftTitle:{         // 右图片左文字
            // 左右布局要重新计算图片宽度和文本高度 比例要按照相反的1：3计算
            iconImgFrame.size.width = self.frame.size.width * titleLabel_Proportion ;
            titleFrame.size.height = self.frame.size.height - self.itemModel.componentMargin.top - self.itemModel.componentMargin.bottom;
            titleFrame.size.width = self.frame.size.width - // 图片的右下终点坐标 + 边距 + 组件右边距极限
        ...
        case LZB_TabBarItemLayoutStylePicture:{                       // 单图片占满全部
        ...
        case LZB_TabBarItemLayoutStyleTitle:{                         // 单标题占满全部
...
- (void)startStrringConfigAnimation{
    ...
    switch (self.itemModel.interactionEffectStyle) {
        case LZB_TabBarInteractionEffectStyleNone:{ // 无
        }break;
        case LZB_TabBarInteractionEffectStyleSpring:{ // 放大放小
        ...
        case LZB_TabBarInteractionEffectStyleShake:{ // 摇动
            animation.keyPath = @"transform.rotation";
            CGFloat angle = M_PI_4 / 10;
            animation.values = @[@(-angle), @(angle), @(-angle)];
            animation.duration = 0.2f;
        }break;
        case LZB_TabBarInteractionEffectStyleAlpha:{ // 透明
            animation.keyPath = @"opacity";
            animation.values = @[@1.0,@0.7,@0.5,@0.7,@1.0];
            animation.duration = 0.6;
        }break;
        case LZB_TabBarInteractionEffectStyleCustom:{ // 自定义动画
...
#pragma mark - SET/GET
- (void)setItemModel:(LZB_TabBarConfigModel *)itemModel{
...
#pragma mark - 懒加载
- (LZB_TabBarBadge *)badgeLabel{
    if (!_badgeLabel) {
        _badgeLabel = [LZB_TabBarBadge new];
...
@implementation LZB_TabBarConfigModel

- (instancetype)init{
    self = [super init];
    if (self) { // 设置初始默认值
        // 默认标题正常颜色
        self.normalColor = KMAINFFFF;
        // 默认选中标题颜色
        self.selectColor = kMAIN9966;
        // 默认凸出 20
