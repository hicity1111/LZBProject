...
#define LZBINTROL_SCREENSIZE \
([[UIScreen mainScreen] respondsToSelector:@selector(nativeBounds)] ? CGSizeMake([UIScreen mainScreen].nativeBounds.size.width/[UIScreen mainScreen].nativeScale,[UIScreen mainScreen].nativeBounds.size.height/[UIScreen mainScreen].nativeScale) : [UIScreen mainScreen].bounds.size)


@interface LZBBaseIntroductionCell : UICollectionViewCell

@property (nonatomic, strong) UIImageView *imgView;

@end

@implementation LZBBaseIntroductionCell
...
@interface LZBIntroductionView ()<UICollectionViewDelegate, UICollectionViewDataSource>
{
...
static NSString *LZBIntroductionViewCellIdentifier = @"LZBIntroductionViewCellIdentifier";

@implementation LZBIntroductionView
...
- (instancetype)initWithImgCount:(NSUInteger)count andDataSource:(id <LZBIntroductionDataSource>)dataSource
{
    self = [super init];
    if (self) {
        self.imgCount = count;
...
{
    CGRect fullFrame = CGRectMake(0, 0, LZBINTROL_SCREENSIZE.width, LZBINTROL_SCREENSIZE.height);
    ...
    flowLayout.itemSize = LZBINTROL_SCREENSIZE;
    ...
    if (!self.dataSource) {
        [_introlCollectionView registerClass:[LZBBaseIntroductionCell class] forCellWithReuseIdentifier:LZBIntroductionViewCellIdentifier];
    }
    
    _pageControl = [[UIPageControl alloc] init];
    _pageControl.numberOfPages = self.imgCount;
    _pageControl.pageIndicatorTintColor = [UIColor colorWithHex:@"#E8E8E8"];
    _pageControl.currentPageIndicatorTintColor = KMAIN00A2;
    if (CGRectEqualToRect(self.pageControl.frame, CGRectZero)) {
        _pageControl.bounds = CGRectMake(0, 0, 200, 30);
        _pageControl.center = CGPointMake(self.center.x, CGRectGetMaxY(fullFrame) - 62*kWidthScale);
    }
    [_pageControl addTarget:self action:@selector(handlePageControlAction:) forControlEvents:UIControlEventValueChanged];
    [self addSubview:_pageControl];
    
    UIImageView *topImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 193*kWidthScale, 64*kWidthScale)];
    topImage.image = IMAGE_NAMED(@"pic_qidongye_one_topbg");
    [self addSubview:topImage];
    
    UIImageView *bottomImage = [[UIImageView alloc] initWithFrame:CGRectMake(kScreenWidth - 260*kWidthScale, kScreenHeight - 62 * kWidthScale, 260*kWidthScale, 62*kWidthScale)];
    bottomImage.image = IMAGE_NAMED(@"pic_guide_page1_topbg2");
...
{
    float width = LZBINTROL_SCREENSIZE.width;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        LZBBaseIntroductionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:LZBIntroductionViewCellIdentifier forIndexPath:indexPath];
